...
private const val NO_DATA = -1.0
class Rainfall {

    private fun getListDouble(town: String, str: String): List<Double>? {
        return str.split("\n")
            .firstOrNull() { it.contains(Regex("""${town}:""")) }
            ?.drop(town.length + 1)
            ?.split(",")
            ?.map { it.drop(4).toDouble() }
    }

    fun mean(town: String, string: String): Double {
        return getListDouble(town, string)
            ?.average() ?: NO_DATA
    }

    fun variance(town: String, string: String): Double {
        val mean = mean(town, string)
        return getListDouble(town, string)
            ?.map { (it - mean).pow(2) }
            ?.let { list -> list.sumOf { it / list.size } } ?: NO_DATA
    }
...
fun main() {
    println(Rainfall().mean("London", Rainfall.data))
    println(Rainfall().variance("London", Rainfall.data))
    //"dima\nLena\nKolya".split("\n").also { println(it[1]) }
}
